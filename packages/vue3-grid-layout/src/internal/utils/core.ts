...
/**
 * clone a layout from target layout
 * @param layout target layout
 * @returns cloned layout
 */
export function cloneLayout(targetLayout: Layout): Layout {
	const newLayout = Array(targetLayout.length)
	for (let i = 0, len = targetLayout.length; i < len; i++) {
		newLayout[i] = cloneLayoutItem(targetLayout[i])
	}
	return newLayout
}
...
export function compact(
	...
	minPositions?: Record<string, LayoutItem>
): Layout {
	...
	for (let i = 0, len = sorted.length; i < len; i++) {
		let layoutItem = sorted[i]

		// Don't move static elements
		if (!layoutItem.static) {
			layoutItem = compactItem(compareWith, layoutItem, verticalCompact, minPositions)
		...
		out[layout.indexOf(layoutItem)] = layoutItem
...
export function correctBounds(layout: Layout, bounds: { cols: number }): Layout {
	...
	for (let i = 0, len = layout.length; i < len; i++) {
		...
		// Overflows right
		if (layoutItem.x + layoutItem.w > bounds.cols) layoutItem.x = bounds.cols - layoutItem.w
		// Overflows left
		if (layoutItem.x < 0) {
			layoutItem.x = 0
			layoutItem.w = bounds.cols
		}
		if (!layoutItem.static) collidesWith.push(layoutItem)
		else {
			// If this is static and collides with other statics, we must move it down.
			// We have to do something nicer than just letting them overlap.
			while (getFirstCollision(collidesWith, layoutItem)) {
				layoutItem.y++
			}
		}
	}
...
export function setTransform(top: number, left: number, width: number, height: number): CssStyle {
...
export function setTopLeft(top: number, left: number, width: number, height: number): CssStyle {
	return {
		top: top + 'px',
		left: left + 'px',
		width: width + 'px',
 ...
 */
export function setTopRight(top: number, right: number, width: number, height: number): CssStyle {
	return {
		top: top + 'px',
		right: right + 'px',
		width: width + 'px',
...
export function sortLayoutItemsByRowCol(layout: Layout): Layout {
	return layout.slice().sort(function (a, b) {
		if (a.y === b.y && a.x === b.x) {
			return 0
		}
...
export function autoBindHandlers(el: Record<string, any>, fns: Array<string>): void {
 ...
 * @returns {string}
 */
export function createMarkup(cssObj: CssStyle) {
	const keyValuesArray = Object.keys(cssObj).map((key: any) => {
		const value = cssObj[key]
		return [hyphenate(key), addPx(key, value)].join(':')
	})
	return keyValuesArray.length ? keyValuesArray.join(';') : ''
}
 ...
 * @returns {*}
 */
export function addPx(name: keyof typeof IS_UNITLESS | keyof CSSStyleDeclaration, value: number) {
